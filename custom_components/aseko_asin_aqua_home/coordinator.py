...
class AsekoCoordinator(DataUpdateCoordinator[DecodedData]):
        ...
        try:
            ...
            while chunk := await reader.read(4096):
                ...
                if self.options["protocol_debug"]:
                    _LOGGER.debug(
                        "ASEKO pending buffer=%d detected frame length=%s",
                        parser.pending_bytes,
                        parser.detected_frame_length,
                    )
    ...
    def _record_parser_events(self, parser: FrameBuffer) -> None:
        ...
        for event in parser.events:
            if self.options["protocol_debug"]:
                _LOGGER.debug(
                    "ASEKO frame candidate accepted=%s offset=%d detected_length=%s reason=%s hex=%s",
                    event.accepted,
                    event.offset,
                    event.detected_frame_length,
                    event.reason,
                    event.candidate_hex,
                )
            if self.options["capture_enabled"]:
    ...
    def _capture_event(timestamp: str, event: CandidateEvent) -> dict[str, Any]:
        return {
            "timestamp": timestamp,
            "type": "frame_candidate",
            "accepted": event.accepted,
            "offset": event.offset,
            "detected_frame_length": event.detected_frame_length,
            "reason": event.reason,
